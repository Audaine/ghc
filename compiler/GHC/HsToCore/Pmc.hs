...
* We keep track of the set of values that can reach a particular program point
  (often loosely referred to as "Covered set")
  in 'GHC.HsToCore.Monad.dsl_nablas :: LdiNablas'.
...
(LDI1) During pattern-match checking, we need to make an auxiliary call the Hs
  desugarer to get the Core of a let-binding or where-clause; see
    - desugarPatBind
    - dsesugarGRHSs
    - desugarEmptyCase
    - desugarMatches
  all of which are Pmc-specific, but which ultimately call `dsLExpr`
  The PMC solver subsequently does light analysis on the thus-constructed
  CoreExpr in `addCoreCt`,  with the goal to find literals and data constructors.

  Now, we don't want to run the coverage checker when doing this auxiliary
  desugaring! Efficiency is one concern, but also a lack of properly set up
  long-distance information might trigger warnings that we normally wouldn't emit.
  ...
  any long-distance info around; we are simply degsugaring to Core. We set
  `dsl_nablas` to `NoPmc` in `dontDoPmc`.

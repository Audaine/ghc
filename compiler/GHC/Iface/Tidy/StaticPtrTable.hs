...
(SF5) One tricky wrinkle: surprisingly, the same expression
   can be desugared more than once: see wrinkle (LDI1) in
   Note [Long-distance information] in GHC.HsToCore.Pmc. We only want
   to emit one set of bindings, of course.  So we test the `ldi_nablas`
   field to see if we are in the pattern-match checking code, and if
   so don't emit the static bindings.
